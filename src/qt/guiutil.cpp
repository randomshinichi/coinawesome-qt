...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("fibocoin"))
        return false;
...
{
    // Convert fibocoin:// to fibocoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("fibocoin://"))
    {
        uri.replace(0, 12, "fibocoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Fibocoin.lnk";
}
...
{
    return GetAutostartDir() / "fibocoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Fibocoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  fibocoin-qt [" + tr("command-line options") + "]                     " + "\n";
